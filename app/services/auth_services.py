...
auth_scheme=OAuth2PasswordBearer(tokenUrl="auth/log_in", auto_error=False)
...
def create_user(user:user_sign_up,db:Session):
    ...
    new_user=User(name=user.name,email=user.email,hashed_password=hashed_password,avatar=user.avatar,join_date=ist_datetime,income_goal=45000,savings_goal=10000,currency=user.currency)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
...
def create_access_token(id:int,email:str,expiry:timedelta|None=None)->str:
    encodings={'sub':email,'id':id,'type':'access'}
    if  expiry:
...
def create_refresh_token(id:int,email:str)->str:
    encodings={'sub':email,'id':id,'type':'refresh','exp':datetime.now(timezone.utc)+timedelta(days=7)}
    return jwt.encode(encodings,SECRET_KEY,ALGORITHM)
...
async def get_current_user(request:Request,db:db_dependancy,token:Annotated[Optional[str],Depends(auth_scheme)]=None):
    token = token or get_token_from_request(request)
    if not token:
        raise Inavlid_credentials_exception
